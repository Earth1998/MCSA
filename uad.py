...
class AdversarialGenerator:
    ...
    # 🌟 修改 1：接收参数 t
    def generate(self, tok, rna, label, device, t=0):
        self.model.eval()
        ...
        for _ in range(self.steps):
            ...
            # 🌟 修改 2：传入 t
            x, _ = self.model.drug_encoder.encoder(adv_emb, mask, mask_attn_embed=None, t=t)
            drug_repr = self.model.drug_encoder.fc(x[:, 0])
            
            cell_repr, _ = self.model.pgim(adv_rna)
            # 🌟 修改 3：传入 t，解决原本的 TypeError 报错
            cell_repr = self.model.cell_encoder(cell_repr, t)
            
            v = torch.cat([drug_repr, cell_repr], dim=1)
            # 🌟 修改 4：传入 t
            v = v + self.model.pmoe(v, t=t)
...
# 🌟 修改 5：接收参数 t
def train_wgan_gp_step(model, netG, netD, optimizer_G, optimizer_D, adv_gen, batch, device, t=0, lambda_gp=10.0):
    """
    ...
    with torch.no_grad():
        # 🌟 修改 6：传入 t
        c_repr_real = model.get_emb(tok, rna, t=t)  # Real Features

    # 2. 生成连续空间的对抗样本 (adv_emb, adv_rna)
    # 注意：此处不能放在 torch.no_grad() 内，因为生成对抗样本需要计算 PGD 梯度
    # 🌟 修改 7：调用 adv_gen 时传入 t
    adv_emb, adv_rna, mask = adv_gen.generate(tok, rna, label, device, t=t)

    # 3. 提取对抗样本经由 Encoder 和 PMOE 后的特征向量作为 Fake Feature
    # 提取特征时再次使用 torch.no_grad()，确保对抗样本特征提取过程不会更新 model 参数
    with torch.no_grad():
        # 🌟 修改 8：传入 t
        x, _ = model.drug_encoder.encoder(adv_emb, mask, mask_attn_embed=None, t=t)
        drug_repr_adv = model.drug_encoder.fc(x[:, 0])
        cell_repr_adv, _ = model.pgim(adv_rna)
        # 🌟 修改 9：传入 t
        cell_repr_adv = model.cell_encoder(cell_repr_adv, t)
        
        c_repr_adv = torch.cat([drug_repr_adv, cell_repr_adv], dim=1)
        # 🌟 修改 10：传入 t
        c_repr_adv = c_repr_adv + model.pmoe(c_repr_adv, t=t) # 对抗样本的特征向量
