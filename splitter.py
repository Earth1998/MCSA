...
def desc_splitter(shuffle=False, batch_size=3):
    gdsc2_rma = pd.read_csv("data/gdsc_csv/gdsc2_rma.csv")
    gdsc2_rma = gdsc2_rma.dropna(subset=["TCGA_DESC"])
    gdsc2_rma = gdsc2_rma[~gdsc2_rma["TCGA_DESC"].isin(["OTHER", "UNCLASSIFIED"])]
    TCGA_DESC = gdsc2_rma["TCGA_DESC"].value_counts().index.to_list()
    index_list = []
    if shuffle:
        random.shuffle(TCGA_DESC)
    for desc in batched(TCGA_DESC, batch_size):
        df = gdsc2_rma[gdsc2_rma["TCGA_DESC"].isin(list(desc))]
        if len(set(df["TCGA_DESC"])) > 1:
            train_df, temp_df = train_test_split(
                df,
                test_size=0.2,
                stratify=df["TCGA_DESC"],
            )
            val_df, test_df = train_test_split(
                temp_df,
                test_size=0.5,
                stratify=temp_df["TCGA_DESC"],
            )
        else:
            train_df, temp_df = train_test_split(
                df,
