...
class GDSCDataset(Dataset):
    def __init__(self, version, platform, geneset_file, binary):
        ...
        if platform == "rma":
            ...
            print(gdsc_df)
            median_value = gdsc_df.groupby("smiles")[binary].transform("median")
            gdsc_df["label"] = (gdsc_df[binary] < median_value).astype(int)
            self.smiles = gdsc_df["smiles"].to_list()
            self.cells = ["DATA." + str(idx) for idx in gdsc_df["COSMIC_ID"].to_list()]
            self.labels = gdsc_df["label"].to_list()
        elif platform == "tpm":
